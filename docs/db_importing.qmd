...
llms-description: "Import transcripts into databases from a variety of sources"
---
...
2. [Arize Phoenix](#arize-phoenix), [LangSmith](#langsmith), [Logfire](#logfire), [MLFlow](#mlflow), [W&B Weave](#wb-weave), and [Datadog](#datadog): Read transcript data from LLM observability platforms.

3. [Claude Code](#claude-code): Read transcript data from local Claude Code sessions.

4. [DataDog](#datadog): Read transcript data from local Claude Code sessions.
...
[Datadog LLM Observability](https://www.datadoghq.com/product/llm-observability/) provides monitoring and tracing for LLM applications. Scout can import transcripts from Datadog LLM Observability traces via the [Export API](https://docs.datadoghq.com/llm_observability/setup/api/), using httpx directly with no SDK dependency.

Use the `datadog()` transcript source to import traces. You can filter by `ml_app`, time range, specific trace ID, span kind, or tags:

```python
from inspect_scout import transcripts_db
from inspect_scout.sources import datadog

async with transcripts_db("s3://my-transcript-db/") as db:
    await db.insert(datadog(
        ml_app="my-llm-app",
    ))
```

You can also apply more specific filters:
...
async with transcripts_db("s3://my-transcript-db/") as db:
    ...
    await db.insert(datadog(
        ...
        to_time=datetime(2025, 2, 1, tzinfo=timezone.utc),
    ))
    ...
    # Filter by tags
    await db.insert(datadog(
        tags=["env:production", "version:2"],
        limit=100,
    ))

    # Skip short conversations and exclude specific models
    await db.insert(datadog(
        ml_app="my-llm-app",
        min_messages=5,
...
Client-side filters (`min_messages`, `exclude_models`) are applied after transcript assembly, so they can filter on data not available through the Datadog Export API's query parameters.

::: {.callout-note}
## Authentication

Set the `DD_API_KEY` and `DD_APP_KEY` environment variables to authenticate with Datadog. Both an API key and an application key are required. Optionally set `DD_SITE` to specify your Datadog region (defaults to `datadoghq.com`). You can create keys from [Datadog Organization Settings](https://app.datadoghq.com/organization-settings/api-keys).
:::
